...
from bot.behaviors.zerg.build_zerg_structure import BuildZergStructure, _find_near_base
...
def test_build_zerg_structure_skips_when_unaffordable() -> None:
    ai = MagicMock()
    ai.not_started_but_in_building_tracker = MagicMock(return_value=0)
    ai.structure_pending = MagicMock(return_value=0)
    ai.can_afford = MagicMock(return_value=False)
    ai.tech_requirement_progress = MagicMock(return_value=1.0)
    mediator = MagicMock()
    mediator.get_own_structures_dict = {UnitTypeId.SPIRE: []}
