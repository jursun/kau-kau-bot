...
Four fixed tiers, run via their own thin wrapper scripts:
    ...
    Quick      1 game, random map/race, Medium  — python scripts/harness/quick_tier.py
    Smoke      1 map per race (3 games), Easy   — python scripts/harness/smoke_tier.py
    Sanity     1 race per map (7 games), Medium — python scripts/harness/sanity_tier.py
    Regression every map x every race (21), Hard — python scripts/harness/regression_tier.py
...
Reuses `scripts/harness/smoke_common.py` for opening-forcing (the Debug/
test_123 cycle dance) and `run.py` for actual game execution, so none of
that plumbing is duplicated here.
